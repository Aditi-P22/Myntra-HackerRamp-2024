...
const ProfileScreen = ({ route }) => {
  ...
  const { userId: loggedInUserId, setUserId } = useContext(UserType);
  const { userId: profileUserId } = route.params || {};
  const isCurrentUserProfile = loggedInUserId === profileUserId;
    ...
    const fetchProfile = async () => {
      try {
        const response = await axios.get(
          `http://192.168.0.155:3000/profile/${profileUserId}`
        );
  ...
  //   try {
  //     const response = await axios.get(
  //       `http://192.168.0.155:3000/posts/user/${profileUserId}`
  //     );
  ...
  const fetchCollections = async () => {
    try {
      const response = await axios.get(
        `http://192.168.0.155:3000/collections/${profileUserId}`
      );
                ...
                <View style={styles.postContent}>
                  ...
                  <View style={styles.textContainer}>
                    ...
                    <View style={styles.interactionBar}>
                      <TouchableOpacity
                        onPress={() =>
                          post.likes?.includes(profileUserId)
